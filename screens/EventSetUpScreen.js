...
const userRequested = route?.params?.item?.requests?.some(
  ...
  console.log('comment', route?.params?.item?.isFull);

  const toggleEventFullStatus = async eventId => {
    try {
      const response = await axios.post(
        'http://10.0.2.2:8000/toggle-event-full',
        {eventId},
      );
          ...
          <View
            ...
            <View
              ...
              <View
                ...
                <FontAwesome
                  onPress={() =>
                    toggleEventFullStatus(route?.params?.item?._id)
                  }
                  name={
